---
{"dg-publish":true,"permalink":"/Books/三角范畴与导出范畴/","dgPassFrontmatter":true,"created":"2024-06-27T12:22:48.927+08:00","updated":"2024-07-03T11:06:40.812+08:00"}
---
...
好三角中,只要存在可交换的两个态射, 三角态射中第三个态射就存在. 三角态射中只要有两个同构, 第三个就同构.<font color=CadetBlue>(证明过程利用Hom是上同调函子构造正合列, 然后利用[[Books/同调代数#^fivelemma\|五引理]]证明同构).</font> 然而一般的三角射, 不满足一个交换就是三角射.
...
<font color=CadetBlue>证明是好三角: 首先抽出来一个态射, 嵌入一个好三角. 再证明这个好三角和需要证明的存在三角射, 即三个态射满足三个交换图. 其中两个是同构显然, 第三个要证明交换. 如果是三角射, 则三角同构显然, 所以是好三角.</font>
...
<iframe class="quiver-embed" src="https://q.uiver.app/#q=&embed" width="560" height="560" style="border-radius: 8px; border: none;"></iframe>

```tikz 
\usepackage{tikz-cd} \begin{document} \begin{tikzcd}[ampersand replacement=\&,cramped] X \& Y \& {Z'} \& TX \\ X \& Z \& {Y'} \& TX \\ \& {X'} \& {X'} \& TY \\ \& TY \& {TZ'} \arrow["u", from=1-1, to=1-2] \arrow[Rightarrow, no head, from=1-1, to=2-1] \arrow["i", from=1-2, to=1-3] \arrow["v", from=1-2, to=2-2] \arrow["{i'}", from=1-3, to=1-4] \arrow["f", dashed, from=1-3, to=2-3] \arrow[Rightarrow, no head, from=1-4, to=2-4] \arrow["vu", from=2-1, to=2-2] \arrow["k", from=2-2, to=2-3] \arrow["j", from=2-2, to=3-2] \arrow["{k'}", from=2-3, to=2-4] \arrow["g", dashed, from=2-3, to=3-3] \arrow["Tu", from=2-4, to=3-4] \arrow[Rightarrow, no head, from=3-2, to=3-3] \arrow["{j'}", from=3-2, to=4-2] \arrow["{j'}", from=3-3, to=3-4] \arrow["{(Ti)j'}", from=3-3, to=4-3] \arrow["Ti", from=4-2, to=4-3] \end{tikzcd} \end{document} 
```
