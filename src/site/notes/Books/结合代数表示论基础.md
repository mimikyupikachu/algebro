---
{"dg-publish":true,"permalink":"/Books/结合代数表示论基础/","dgPassFrontmatter":true,"created":"2024-06-28T10:03:59.708+08:00","updated":"2024-07-04T09:16:21.186+08:00"}
---
...
**可裂(splits)** 短正合列:  $M=\mathrm{\mathrm{Im}}u\oplus \mathrm{Ker}p=\mathrm{Im}v\oplus \mathrm{Ker}r$. 其中 $pu=1_{L},\ rv=1_{N}$. 等价于 $r$ 有收缩或 $u$ 有截面.
{ #7cfea6}
...
其中,  $e_{ij}=\mathrm{dim}_{K}e_{j}Ae_{i}=\mathrm{dim}_{K}(\mathrm{Hom}_{A}(P(i),P(j)))=\mathrm{dim}_{K}(\mathrm{Hom}_{A}(E(i),E(j)))$
...
**左极小(left mimnimal)**: $A$- 模同态 $f:L \rightarrow M$ 被称为左极小, 如果对任意自同态 $h \in \mathrm{End\ } M$ 都满足 $hf=f$ 是自同构.

**左殆分裂(left almost split)**: $A$- 模同态 $f:L \rightarrow M$ 是左殆分裂的, 满足(1).$f$ 不是截面;(2).对任意不是截面的 $u:L \rightarrow U$ 满足存在<font color=CadetBlue>(不唯一)</font>态射 $u':M \rightarrow U$ 使得 $u=u'f$.

**右极小(right mimnimal)**: $A$- 模同态 $g:M \rightarrow N$ 被称为右极小, 如果对任意自同态 $k \in \mathrm{End\ } M$ 都满足 $gk=g$ 是自同构.

**右殆分裂(right almost split)**: $A$- 模同态 $g:M \rightarrow N$ 是右殆分裂的, 满足(1).$g$ 不是收缩;(2).对任意不是收缩的 $v:V \rightarrow N$ 满足存在<font color=CadetBlue>(不唯一)</font>态射 $v':V \rightarrow M$ 使得 $v=gv'$.

左极小殆分裂态射由其起点唯一确定, 右极小殆分裂态射由其重点唯一确定.