---
{"dg-publish":true,"permalink":"/Books/结合代数表示论基础/Ⅳ. Auslander-Reiten theory/Ⅳ.1 不可约态射与殆分裂序列/","dgPassFrontmatter":true,"created":"2024-08-05T17:36:37.754+08:00","updated":"2024-10-11T09:18:55.363+08:00"}
---
...
**殆分裂序列(almost split sequence)(AR 序列)(Auslander_Reiten sequence)**:  $\mathrm{mod\ }A$ 中的短正合列 $0 \rightarrow L \stackrel{f}{\longrightarrow} M \stackrel{g}{\longrightarrow} N \rightarrow 0$ 满足 $f$ 是极小左殆分裂态射,  $g$ 是极小右殆分裂态射. 
...
殆分裂序列 $0 \rightarrow L \stackrel{f}{\longrightarrow} M \stackrel{g}{\longrightarrow} N \rightarrow 0$ 是模范畴 $\operatorname{mod}\Lambda$ 中的短正合列, 首尾 $L$ 和 $N$ 不可分解, 序列不可裂. 若有非可裂满的 $\varphi:X\rightarrow N$, 则有 $\psi :X\rightarrow M$ 使得 $\varphi=g\psi$. 此时 $L=\tau N$,  $N=\tau^{-1}L$. 总是存在以不可分解非投射模为终点的AR序列, 总是存在以不可分解非内射模为起点的AR序列.

模范畴中不可分解模之间的不可约态射有四种形式: 殆分裂序列左边的, 殆分裂序列右边的, rad到P的, I到I/S的.
