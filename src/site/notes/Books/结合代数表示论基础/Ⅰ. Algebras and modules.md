---
{"dg-publish":true,"permalink":"/Books/结合代数表示论基础/Ⅰ. Algebras and modules/","dgPassFrontmatter":true,"created":"2024-07-05T15:03:51.419+08:00","updated":"2024-07-07T19:28:03.160+08:00"}
---
...
**多余子模(superfluous submodule)**: 若对任意 $M$ 的子模 $X$, $L+X=M\implies X+M$, 则称子模 $L$ 是 $M$ 的多余子模.

若 $M$ 是有限维代数上的有限生成模, 则 $\mathrm{rad\ }M$ 是 $M$ 的多余子模.

**极小满射(minimal epimorphism)**: $h:M\rightarrow N\rightarrow 0$ 是极小的, 如果 $\mathrm{Ker\ }h$ 是 $M$ 的多余子模.
...
**极小投射表现(minimal projective presentation)**: 正合列 $P_{1}\stackrel{p_{1}}{\longrightarrow}P_{0}\stackrel{p_{0}}{\longrightarrow}M\rightarrow 0$ 中, $P_{1}\stackrel{p_{1}}{\longrightarrow}\mathrm{Ker\ }p_{0}$ 和 $P_{0}\stackrel{p_{0}}{\longrightarrow}M$ 是投射盖.
{ #5475d0}
...
**内射包络(injective envelope)**: $u:0\rightarrow L\rightarrow E$ 被称为 $E$ 的内射包, 如果 ① $P$ 是内射模; ② $u$ 是单射; ③ $u$ 是极小的.
