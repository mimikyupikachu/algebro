---
{"dg-publish":true,"permalink":"/Books/三角范畴与导出范畴/Ⅲ. 商范畴/","dgPassFrontmatter":true,"created":"2024-07-23T13:31:58.989+08:00","updated":"2024-07-30T22:08:17.950+08:00"}
---
...
**厚子范畴(thick subcategory)**: 三角范畴 $(\mathcal{K},[1])$ 的子范畴 $\mathcal{B}$ 被称为 $\mathcal{K}$ 的厚子范畴, 如果 $\mathcal{K}$ 中满足 $Z \in \mathcal{B}$ 的好三角 $X\stackrel{f}{\longrightarrow}Y\rightarrow Z\rightarrow X[1]$ 且 $f$ 能通过 $\mathcal{B}$ 中某对象 $W$ 分解, 则 $X,Y \in \mathcal{B}$.

 $\mathcal{K}$ 的三角子范畴 $\mathcal{B}$ 是厚子范畴当且仅当对直和**项**封闭. 即若 $X_1 \oplus X_2 \in \mathcal{B}$, 则 $X_1 \in \mathcal{B},X_2 \in B$.
