---
{"dg-publish":true,"permalink":"/Books/三角范畴与导出范畴/Ⅰ. 三角范畴/","dgPassFrontmatter":true,"created":"2024-06-27T12:22:48.927+08:00","updated":"2024-07-10T08:13:00.768+08:00"}
---
...
**上同调函子(cohomological functor)**：设 $(\mathcal{C},T,\mathcal{E})$ 是预三角范畴,  $\mathcal{A}$ 是[[Books/范畴论中若干基本概念和结论/9 Abel 范畴#^abeliancat\|阿贝尔范畴]]. 上同调函子 $H:\mathcal{C}\rightarrow\mathcal{A}$ 是指对 $\mathcal{C}$ 中任意好三角 $X\stackrel{u}{\longrightarrow}Y\stackrel{v}{\longrightarrow}Z\stackrel{w}{\longrightarrow}TX$ 均能构造正合列:
...
**三角核(triangle kernel)与三角余核(triangle cokernel)**: 三角范畴中有好三角 $X\stackrel{u}{\longrightarrow}Y\stackrel{v}{\longrightarrow}Z\stackrel{w}{\longrightarrow}TX$. 则称 $(X,u)$ 是 $v$ 的三角核, $(TX,w)$ 是 $v$ 的三角余核. 利用 TR3 容易证明, 三角核与三角余核满足类似于核的[[Books/范畴论中若干基本概念和结论/2 核与余核#^kernel\|分解性]]与余核的[[Books/范畴论中若干基本概念和结论/2 核与余核#^cokernel\|分解性]]<font color=CadetBlue>(只不过这个分解只是存在, 未必唯一)</font>. 在同构的意义下三角核与三角余核是唯一的.<font color=CadetBlue>(这个概念只在好三角中有意义.)</font>
...
**三角等价(triangle-equivalence)**:  $F$ 是[[Books/范畴论中若干基本概念和结论/4 范畴的等价#^ef290f\|等价函子]].
**三角同构(triangle-isomorphism)**:  $F$ 是[[Books/范畴论中若干基本概念和结论/4 范畴的等价#^ecf8f4\|同构函子]].
...
[[Books/范畴论中若干基本概念和结论/8 伴随对#^b5725a\|伴随对]]中一个是三角函子当且仅当另一个也是三角函子.

## 1.7 基变换和余基变换

[[Books/三角范畴与导出范畴/Ⅰ. 三角范畴#^01f47f\|TR4]]的等价定义: 若 $(\mathcal{C},[1],\mathcal{E})$ 是**预三角范畴**, 则下述等价:
$\qquad$ ①: TR4;
$\qquad$ ②: TR4': 对于 $\mathcal{C}$ 中态射序列 $A\stackrel{f_{1}}{\longrightarrow}B\stackrel{f_{2}}{\longrightarrow}C$ 存在如下交换图:<br/> <div style="text-align: center;"> $\begin{CD}A@>f_{1}>>B@>g_{1}>>X@>h_{1}>>A[1]\\@| @Vf_2VV@V\alpha VV@|\\A@>f_{2}f_{1}>>C@>g_{3}>>Y@>h_{3}>>A[1]\\@.@Vg_{2}VV@V\beta VV@Vf_{1}[1]VV\\@.Z@=Z@>h_{2}>>B[1]\\@.@Vh_{2}VV@V\gamma VV\\@.B[1]@>g_{1}[1]>>X[1]\end{CD}$</div><br/> 其中前两行和中间两列均是好三角.
$\qquad$ ③: **基变换Ⅰ(base change)**: 对于 $\mathcal{C}$ 中好三角 $A\stackrel{f}{\longrightarrow}B\stackrel{g}{\longrightarrow}C\stackrel{h}{\longrightarrow}A[1]$ 和 $\mathcal{C}$ 中态射 $\varepsilon:C'\rightarrow C$, 存在如下交换图:<br/> <div style="text-align: center;"> $\begin{CD}@.E@=E\\@.@V\alpha VV @V\delta VV\\A@>f'>>B'@>g'>>C'@>h'>>A[1]\\@| @V\beta VV@V\varepsilon VV@|\\A@>f>>B@>g>>C@>h>>A[1]\\@.@V\gamma VV@V\eta VV@Vf'[1]VV\\@.E[1]@=E[1]@>-\alpha[1]>>B'[1]\end{CD}$</div><br/> 其中中间两行和中间两列均是好三角.
$\qquad$ ④**基变换Ⅱ**: 对于 $\mathcal{C}$ 中好三角 $A\stackrel{f}{\longrightarrow}B\stackrel{g}{\longrightarrow}C\stackrel{h}{\longrightarrow}A[1]$、$\mathcal{C}$ 中态射 $\varepsilon:C'\rightarrow C$ 以及 $\mathcal{C}$ 中给定的两个好三角 $E \stackrel{\delta}{\longrightarrow}C'\stackrel{\varepsilon}{\longrightarrow}C\stackrel{\eta}{\longrightarrow}E[1]$ 及 $A\stackrel{f'}{\longrightarrow}B'\stackrel{g'}{\longrightarrow}C'\stackrel{h\varepsilon}{\longrightarrow}A[1]$, 存在如下交换图:<br/> <div style="text-align: center;"> $\begin{CD}@.E@=E\\@.@V\alpha VV @V\delta VV\\A@>f'>>B'@>g'>>C'@>h\varepsilon>>A[1]\\@| @V\beta VV@V\varepsilon VV@|\\A@>f>>B@>g>>C@>h>>A[1]\\@.@V\gamma VV@V\eta VV@Vf'[1]VV\\@.E[1]@=E[1]@>-\alpha[1]>>B'[1]\end{CD}$</div><br/> 其中中间两行和中间两列均是好三角.
$\qquad$ ⑤**余基变换Ⅰ(cobase change)**: 对于 $\mathcal{C}$ 中好三角 $A\stackrel{f}{\longrightarrow}B\stackrel{g}{\longrightarrow}C\stackrel{h}{\longrightarrow}A[1]$ 和 $\mathcal{C}$ 中态射 $\alpha:A\rightarrow A'$, 存在如下交换图:<br/> <div style="text-align: center;"> $\begin{CD}@.F@=F\\@.@V\eta VV @V\varepsilon VV\\C[-1]@>-h[-1]>>A@>f>>B@>g>>C\\@| @V\alpha VV@V\beta VV@|\\C[-1]@>-h'[-1]>>A'@>f'>>B'@>g'>>C\\@.@V\gamma VV@V\delta VV@V-hVV\\@.F[1]@=F[1]@>-\eta[1]>>A[1]\end{CD}$</div><br/> 其中中间两行和中间两列均是好三角.
$\qquad$ ⑤**余基变换Ⅱ**: 对于 $\mathcal{C}$ 中好三角 $A\stackrel{f}{\longrightarrow}B\stackrel{g}{\longrightarrow}C\stackrel{h}{\longrightarrow}A[1]$、$\mathcal{C}$ 中态射 $\alpha:A\rightarrow A'$以及 $\mathcal{C}$ 中给定的两个好三角 $F \stackrel{\eta}{\longrightarrow}A\stackrel{\alpha}{\longrightarrow}A'\stackrel{\gamma}{\longrightarrow}F[1]$ 及 $F\stackrel{\varepsilon}{\longrightarrow}B\stackrel{\beta}{\longrightarrow}B'\stackrel{\delta}{\longrightarrow}F[1]$, 存在如下交换图:<br/> <div style="text-align: center;"> $\begin{CD}@.F@=F\\@.@V\eta VV @V\varepsilon VV\\C[-1]@>-h[-1]>>A@>f>>B@>g>>C\\@| @V\alpha VV@V\beta VV@|\\C[-1]@>-\alpha[-1]h[-1]>>A'@>f'>>B'@>g'>>C\\@.@V\gamma VV@V\delta VV@V-hVV\\@.F[1]@=F[1]@>-\eta[1]>>A[1]\end{CD}$</div><br/> 其中中间两行和中间两列均是好三角.
...
$$\begin{CD}A_{1}@>x_{1}>>B_{1}\\@Va_{1}VV@VVb_{1}V\\A_{2}@>>x_{2}>B_{2}\end{CD}$$以及 $\mathcal{C}$ 中给定的四个好三角<br/> 
$$\begin{split}A_{1}\stackrel{x_{1}}{\longrightarrow}B_{1}\stackrel{y_{1}}{\longrightarrow}&C_{1}\stackrel{x_{1}}{\longrightarrow}A_{1}[1]\\A_{2}\stackrel{x_{2}}{\longrightarrow}B_{2}\stackrel{y_{2}}{\longrightarrow}&C_{2}\stackrel{x_{2}}{\longrightarrow}A_{2}[1]\\A_{1}\stackrel{x_{1}}{\longrightarrow}A_{2}\stackrel{y_{2}}{\longrightarrow}&A_{3}\stackrel{x_{3}}{\longrightarrow}A_{1}[1]\\B_{1}\stackrel{x_{1}}{\longrightarrow}B_{2}\stackrel{y_{2}}{\longrightarrow}&B_{3}\stackrel{x_{3}}{\longrightarrow}B_{1}[1]\end{split}$$<br/> 存在下列态射图<br/> 
$$\begin{CD}A_1@>x_1>>B_1@>y_1>>C_1@>z_1>>A_1[1]\\@VVa_1V@VVb_1V@VVc_1V@VVa_1[1]V\\A_2@>x_2>>B_2@>y_2>>C_2@>z_2>>A_2[1]\\@VVa_2V@VVb_2V@VVc_2V@VVa_2[1]V\\A_3@>x_3>>B_3@>y_3>>C_3@>z_3>>A_3[1]\\@VVa_3V@VVb_3V@VVc_3V@VV-a_3[1]V\\A_1[1]@>x_1[1]>>B_1[1]@>y_1[1]>>C_1[1]@>-z_1[1]>>A_1[2]\end{CD}$$ <br/> 其中四行四列都是好三角, **除了右下角以外都交换**.